...
fn test_jsonld_node_creation_and_validation() {
    ...
    let valid_node = create_test_node(
        "iri://task/test/1",
        "PlanNode",
        HashMap::from([("summary", json!("测试计划")), ("confidence", json!(0.85))]),
    );
...
fn test_entity_alignment_and_graph_merge() {
    ...
    let sparql = format!("SELECT ?p ?o WHERE {{ <{}> ?p ?o . }}", shared_id);
...
fn test_multi_type_query() {
    ...
    blackboard
        .write_node("iri://test/plan", &plan_node, &config)
        .unwrap();
    blackboard
        .write_node("iri://test/exec", &exec_node, &config)
        .unwrap();
    blackboard
        .write_node("iri://test/multi", &multi_node, &config)
        .unwrap();

    let plan_nodes = blackboard
        .query_by_types(&["PlanNode".to_string()])
        .unwrap();
    assert_eq!(plan_nodes.len(), 2, "应该找到2个PlanNode类型的节点");

    let exec_nodes = blackboard
        .query_by_types(&["ExecutionResult".to_string()])
        .unwrap();
...
fn test_named_graph_isolation() {
    ...
    blackboard
        .write_node_to_graph("iri://test/plan", &plan_node, "system:plan", &config)
        .unwrap();
    blackboard
        .write_node_to_graph("iri://test/exec", &exec_node, "system:execution", &config)
        .unwrap();

    let plan_results = blackboard.query_graph("system:plan", "?s ?p ?o").unwrap();
    assert!(!plan_results.is_empty(), "plan图应该有节点");

    let exec_results = blackboard
        .query_graph("system:execution", "?s ?p ?o")
        .unwrap();
    assert!(!exec_results.is_empty(), "execution图应该有节点");

    let all_results = blackboard
        .query("SELECT ?s WHERE { { ?s a ?type } UNION { GRAPH ?g { ?s a ?type } } }")
        .unwrap();
    assert!(
        all_results.len() >= 2,
        "全局查询应该返回所有节点, 实际: {}",
...
fn test_token_budget_control() {
    ...
    for i in 0..10 {
        ...
        .to_string();
        blackboard
            .write_node(&format!("iri://test/node/{}", i), &node, &config)
            .unwrap();
    }
...
fn test_sparql_query_on_jsonld_nodes() {
    ...
    blackboard
        .write_node("iri://task/1", &node1, &config)
        .unwrap();
    blackboard
        .write_node("iri://task/2", &node2, &config)
        .unwrap();
...
fn test_batch_write_to_graphs() {
    ...
    let nodes = vec![
        (
            "iri://test/1".to_string(),
            json!({"@id": "iri://test/1", "@type": "Plan"}).to_string(),
            "system:plan".to_string(),
        ),
        (
            "iri://test/2".to_string(),
            json!({"@id": "iri://test/2", "@type": "Execution"}).to_string(),
            "system:execution".to_string(),
        ),
        (
            "iri://test/3".to_string(),
            json!({"@id": "iri://test/3", "@type": "Review"}).to_string(),
...
fn test_node_tags_and_metadata() {
    ...
    blackboard
        .write_node("iri://test/tagged", &node, &config)
        .unwrap();
...
fn test_performance_projection() {
    ...
    for i in 0..30 {
        ...
        .to_string();
        blackboard
            .write_node(&format!("iri://task/perf/node/{}", i), &node, &config)
            .unwrap();
    }
