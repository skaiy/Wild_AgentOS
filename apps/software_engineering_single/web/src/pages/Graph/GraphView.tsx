...
import { Card, Input, Button, Space, Drawer, Descriptions, Empty, Spin } from 'antd';
...
const GraphView: React.FC<GraphViewProps> = ({ projectId }) => {
  ...
  const renderGraph = (data: GraphData) => {
    ...
    const g6Data = {
      nodes: data.nodes.map((node) => ({
        ...
        label: node.label,
        style: {
          fill: nodeTypeColors[node.type] || nodeTypeColors.default,
          stroke: nodeTypeColors[node.type] || nodeTypeColors.default,
        },
        originalData: node,
      })),
  ...
  const handleSearch = () => {
    ...
    const foundNode = graphData.nodes.find(
      (n) => n.label.toLowerCase().includes(searchText.toLowerCase()) || n.id.includes(searchText)
    );
            ...
            <Descriptions column={1} bordered size="small">
              ...
              {selectedNode.iri && (
                <Descriptions.Item label="IRI">
                  <code className={styles.iriCode}>{selectedNode.iri}</code>
                </Descriptions.Item>
              )}
              {selectedNode.properties && (
                <Descriptions.Item label="属性">
                  <pre className={styles.propertiesPre}>
