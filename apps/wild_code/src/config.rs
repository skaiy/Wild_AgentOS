...
impl CliConfig {
    ...
    /// 从环境变量加载 MCP 服务器配置。
    /// 支持两种格式：
    /// 1. `WILD_AGENT_OS_MCP_SERVERS` JSON 数组（兼容旧名 `GLIDING_HORSE_MCP_SERVERS`）：
    ...
    fn load_mcp_servers() -> Vec<McpServerEntry> {
        ...
        // 优先从 JSON 环境变量加载
        if let Ok(json_str) = std::env::var("WILD_AGENT_OS_MCP_SERVERS")
            .or_else(|_| std::env::var("GLIDING_HORSE_MCP_SERVERS"))
        {
