...
impl PromptRegistry {
    ...
    pub fn add_version(&self, mut v: PromptVersion) -> String {
        if v.id.is_empty() {
            v.id = Uuid::new_v4().hyphenated().to_string();
        }
    ...
    pub fn activate(&self, id: &str) -> bool {
        ...
        for mut e in self.versions.iter_mut() {
            e.value_mut().is_active = false;
        }
        if let Some(mut e) = self.versions.get_mut(id) {
            e.value_mut().is_active = true;
        }
    ...
    pub fn resolve(&self, tenant_id: &str, user_id: &str, role: &str) -> Option<ResolvedPrompt> {
        ...
        for e in self.versions.iter() {
            let v = e.value();
            if v.canary_percent == 0 {
                continue;
            }
            let t_ok = v.canary_tenant_ids.is_empty()
                || v.canary_tenant_ids.iter().any(|t| t == tenant_id);
