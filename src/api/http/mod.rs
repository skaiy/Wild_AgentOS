...
fn normalize_git_skill_subpath(path: &str) -> Result<std::path::PathBuf, &'static str> {
...
async fn list_pipeline_runs_handler(Query(q): Query<PipelineRunsQuery>) -> impl IntoResponse {
    let mut runs = load_pipeline_runs();
...
async fn list_prompts_handler(State(state): State<Arc<AppState>>) -> impl IntoResponse {
