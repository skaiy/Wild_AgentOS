...
impl SkillGraphEmbedder {
    ...
    fn compute_coords(&self, skill: &SkillGraphNode) -> Vec<f64> {
        ...
        for value in fp.iter().take(fp.len().min(self.dim.saturating_sub(2))) {
            let scaled = value * 0.2;
            coords.push(scaled);
        }
    ...
    fn tag_fingerprint(&self, tags: &[String], n: usize) -> Vec<f64> {
        ...
        for tag in tags {
            for (i, value) in fp.iter_mut().enumerate().take(n) {
                let h = stable_hash(&format!("{}:{}", i, tag)) as f64;
                *value += (h / u64::MAX as f64) * 2.0 - 1.0;
            }
        }
